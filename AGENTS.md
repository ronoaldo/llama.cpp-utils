# Agent Instructions: llama-launcher

## Core Workflow
- **Start Server**: Run `./llama-launcher` to start the `llama serve` instance on port `8080`.
- **Interact**: Use `./ping <model_alias> [prompt]` to test a model.
...
- `ping`: A CLI utility to send chat completion requests to the server.
